...
## Uploading Code
The Arduino Nano 33 BLE can be programmed using a micro-USB connected to a computer running the Arduino IDE https://www.arduino.cc/en/software (or the cloud editor found here https://app.arduino.cc/). This repository can be cloned (functionality should be at the top of the repository) as a ZIP file and opened using the chosen editor. Choose the folder of the desired sketch and click upload. A more in-depth tutorial for the Arduino IDE can be found here https://support.arduino.cc/hc/en-us/articles/4733418441116-Upload-a-sketch-in-Arduino-IDE.

**Note**: When uploading the sketches that have BLE functionality, note that there are lines that should be commented/uncommented specifically for the left and right devices. These lines of code help identify the devices when attempting to connect to the corresponding iOS app. They are labeled with comments in the code near the top of the main sketch file.
