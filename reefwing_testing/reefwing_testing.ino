...
BLA::Matrix<4> accel_array[200]; // fill this in starting at t_su until receive a t_sd 
...
//MM 
  ...
  if(instances_above_thresh > 2){
    if(accel_array[i-5][3] < z_sd_thresh){
...
float calculate_step_length(BLA::Matrix<3> accel_array[], uint32_t time_stamp_array[], int i_su, int i_sd) { //, uint32_t t_sd, uint32_t t_su){
  ...
  for(int i = i_su; i < i_sd; i++){
    ...
    dt = time_stamp_array[i] / 1000.0; //in seconds instead of ms 
    vx += ax * dt;
    vy += ay * dt;
    vz += az * dt;
